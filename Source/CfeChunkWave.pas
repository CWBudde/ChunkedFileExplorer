...
type
  ...
  TWaveData = class(TCustomMemoryStreamChunk)
  public
    constructor Create;
  ...
  private
    ...
    FDataChunk: TWaveData;
    FUnknownChunks: TChunkList;
    ...
    procedure CheckCreateBextChunk;
...
resourcestring
  ...
  RCFMTChunkDuplicate = 'More than one format chunk found!';
  RCFACTChunkDuplicate = 'More than one fact chunk found!';
  RCDATAChunkDuplicate = 'Only one data chunk supported!';
...
begin
  ...
  begin
    ...
    begin
      ...
      ConvertStreamToChunk(TFileWave.ChunkClassByChunkName(ChunkName), Stream);
    end;
...
class function TWaveData.GetClassChunkName: TChunkName;
begin
  Result := 'data';
end;
...
constructor TFileWave.Create;
begin
  inherited;

  FChunkList.OwnsObjects := False;
  FFormatChunk := TFormatChunk.Create;
  FUnknownChunks := TChunkList.Create;
end;
...
class function TFileWave.CanLoad(Stream: TStream): Boolean;
var
  ChunkID : TChunkName;
begin
  ...
  begin
    ...
    Result := ChunkID = GetClassChunkName;
  end;
end;

class function TFileWave.GetClassChunkName: TChunkName;
begin
  Result := 'RIFF';
end;

procedure TFileWave.CheckHeader(const Stream: TStream);
var
  ChunkName: TChunkName;
begin
  with Stream do
...
procedure TFileWave.ParseStream(const Stream: TStream);
var
  ChunkName: TChunkName;
  ChunkEnd: Cardinal;
begin
  ...
  begin
    ...
    if not Assigned(FDataChunk) then
      raise EWavError.Create(RCStrNoDataChunkFound)
  end;
end;

procedure TFileWave.LoadFromStream(Stream: TStream);
begin
  CheckHeader(Stream);
  ParseStream(Stream);
end;

procedure TFileWave.ReadFormatChunk(const Stream: TStream);
begin
  with Stream do
  begin
    // check whether format chunk has already been created
    if FFormatChunkFound then
      raise Exception.Create(RCFACTChunkDuplicate);
...
procedure TFileWave.ReadFactChunk(const Stream: TStream);
begin
  with Stream do
  begin
...
procedure TFileWave.ReadDataChunk(const Stream: TStream);
var
  DataSize: Cardinal;
...
begin
  with Stream do
    if ctData in ChunksReaded then
      raise EWavError.Create(RCDATAChunkDuplicate)
    else
    begin
      FDataChunk := TWaveData.Create;
      FDataChunk.LoadFromStream(Stream);
      AddChunk(FDataChunk);

      // eventually set total number of samples
      if not Assigned(FFactChunk) then
        FTotalSampleFrames := FDataChunk.ChunkSize div FFormatChunk.BlockAlign
      else if FFormatChunk.FormatTag <> etPCM then
        FTotalSampleFrames := FFactChunk.SampleCount;
    end
end;

procedure TFileWave.CheckCreateCartChunk;
begin
...
procedure TFileWave.CheckCreateBextChunk;
begin
...
procedure TFileWave.ReadBextChunk(const Stream: TStream);
begin
  with Stream do
  begin
...
procedure TFileWave.ReadCartChunk(const Stream: TStream);
begin
  with Stream do
  begin
...
procedure TFileWave.ReadUnknownChunk(const Stream: TStream;
  const ChunkName: TChunkName);
var
  ChunkClass: TDefinedChunkClass;
  DefinedChunk: TDefinedChunk;
  UnknownChunk: TWavUnknownChunk;
begin
  ...
  begin
    UnknownChunk := TWavUnknownChunk.Create;
    UnknownChunk.LoadFromStream(Stream);
    FUnknownChunks.Add(UnknownChunk);
...
initialization
  ...
  TFileWave.RegisterChunks([TFormatChunk, TFactChunk, TQualityChunk, TLabelChunk,
